...
export interface SymbolInfo {
  name: string;
  kind: 'function' | 'class' | 'interface' | 'type' | 'enum' | 'const' | 'variable' | 'export';
  file: string;           // relative path from project root
  startLine: number;
  endLine: number;
  signature?: string;     // e.g. "function evaluate(node: ASTNode): ReiVal"
  exported: boolean;
  jsdoc?: string;
}
...
export interface FileInfo {
  ...
  symbols: SymbolInfo[];
  imports: ImportInfo[];
  description?: string;   // first comment block
}
...
export interface ProjectIndex {
  ...
  symbols: Map<string, SymbolInfo[]>;  // name -> all definitions
  buildTime: Date;
}
...
export class ReiProjectIndexer {
  ...
  private extractFileDescription(lines: string[]): string | undefined {
    ...
    for (const line of lines.slice(0, 30)) {
      ...
      if (trimmed.startsWith('/**')) { inComment = true; descLines.push(trimmed); continue; }
      if (inComment) {
   ...
   * import文を解析
   */
  private parseImports(lines: string[], _relPath: string): ImportInfo[] {
   ...
   */
  private parseSymbols(lines: string[], relPath: string): SymbolInfo[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      let sym: Partial<SymbolInfo> | null = null;
      ...
      if (sym) {
        ...
        symbols.push({
          name: sym.name!,
          kind: sym.kind as SymbolInfo['kind'],
          file: relPath,
   ...
   * JSDocコメントを抽出
   */
  private extractJSDoc(lines: string[], symbolIdx: number): string | undefined {
    if (symbolIdx === 0) return undefined;
   ...
   * シンボル名で検索
   */
  searchSymbol(query: string): SymbolInfo[] {
   ...
   * ファイルの特定行範囲を取得
   */
  getFileLines(relPath: string, startLine: number, endLine: number): string | null {
   ...
   * シンボルの呼び出し元を検索
   */
  findCallers(symbolName: string): CallerInfo[] {
    ...
    for (const [relPath, _fileInfo] of this.index.files) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (line.match(/^(export\s+)?(function|class|interface|type|enum|const)\s/)) continue;
  ...
  getProjectSummary(): string {
    ...
    for (const f of files) {
      ...
      for (const s of exports) {
        ...
        if (s.signature) summary += ` — ${s.signature}`;
   ...
   * テキスト全文検索
   */
  grepProject(pattern: string, maxResults: number = 50): CallerInfo[] {
