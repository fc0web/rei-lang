...
> **注意 (v0.5.3時点):** 上記の4例はv0.3設計時の**構想コード**であり、`空 { 層 N: ... } |> diffuse()` 構文は現在のインタプリタでは未実装です。これらは「Reiが目指す記述スタイル」を示すものであり、そのまま実行はできません。
>
> **現在実際に動くもの:** σ（6属性メタデータ）、trace（依存追跡）、influence（影響度）、entangle（縁起的結合）、will_evolve（意志の自律進化）、bind（関係結合）は**すべてv0.5.3で動作します**。実行可能なベンチマーク（Rei vs TypeScript比較7本、14テスト全パス）は `benchmarks/benchmark-suite.test.ts` をご覧ください。
>
> ```bash
...
- 上記4例の「空/diffuse」構文は将来の実装目標として残しているが、API安定性分類ではExperimentalに位置づけている（`docs/API-STABILITY.md` 参照）。
